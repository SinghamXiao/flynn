...
func (s *Scheduler) findVolume(job *Job, req *ct.VolumeReq) *Volume {
	for _, vol := range s.volumes {
		// skip decommissioned volumes
		if vol.DecommissionedAt != nil {
			continue
		}
...
func (s *Scheduler) HandlePlacementRequest(req *PlacementRequest) {
	...
	if reqs := req.Job.VolumeRequests(); len(reqs) > 0 {
		...
		for i, volReq := range reqs {
			...
			if vol == nil {
				...
				// assign the new volume to req.NewVolumes so
				// that the StartJob goroutine knows this
				// volume needs to be created on the host
				req.NewVolumes = append(req.NewVolumes, vol)
	...
	for _, vol := range req.NewVolumes {
...
func (s *Scheduler) HandleVolumeEvent(e *VolumeEvent) {
	...
	case VolumeEventTypeDestroy:
		// treat destroyed volumes like decommissioned ones
		// (i.e. don't assign them to any jobs)
		if vol, ok := s.volumes[e.Volume.ID]; ok {
			now := time.Now()
			vol.DecommissionedAt = &now
			s.persistVolume(vol)
		}
