...
func (r *VolumeRepo) Decommission(vol *ct.Volume) error {
	tx, err := r.db.Begin()
	if err != nil {
		return err
	}

	err = tx.QueryRow("volume_decommission", vol.ID).Scan(&vol.UpdatedAt, &vol.DecommissionedAt)
	if err != nil {
		tx.Rollback()
		return err
	}
...
func (c *controllerAPI) DecommissionVolume(ctx context.Context, w http.ResponseWriter, req *http.Request) {
	...
	if err := c.volumeRepo.Decommission(&volume); err != nil {
		respondWithError(w, err)
		return
	}
	httphelper.JSON(w, 200, &volume)
