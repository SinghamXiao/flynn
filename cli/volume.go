...
func runVolumeList(args *docopt.Args, client controller.Client) error {
	...
	listRec(w, "ID", "HOST", "ATTACHED JOB", "CREATED", "DECOMMISSIONED")
	for _, v := range volumes {
		...
		listRec(w, v.ID, v.HostID, jobID, created, v.DecommissionedAt != nil)
	}
...
func runVolumeShow(args *docopt.Args, client controller.Client) error {
	vol, err := client.GetVolume(args.String["<id>"])
	if err != nil {
		return err
	}
	if args.Bool["--json"] {
...
func runVolumeDecommission(args *docopt.Args, client controller.Client) error {
	...
	if err := client.DecommissionVolume(vol); err != nil {
		return err
	}
